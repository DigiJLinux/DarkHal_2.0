...
from agentdhal_core.memory import Memory
...
from agentdhal_core.tools import BaseTool, FunctionTool, StaticStreamWorkbench, ToolResult, Workbench
...
class Hal(BaseChatAgent, Component[AssistantAgentConfig]):
    """An agent that provides assistance with tool use.
    The :meth:`on_messages` returns a :class:`~agentdhal_agentchat.base.Response`
    in which :attr:`~agentdhal_agentchat.base.Response.chat_message` is the final
    response message.

    The :meth:`on_messages_stream` creates an async generator that produces
    the inner messages as they are created, and the :class:`~agentdhal_agentchat.base.Response`
    object as the last item before closing the generator.

    The :meth:`BaseChatAgent.run` method returns a :class:`~agentdhal_agentchat.base.TaskResult`
    ...
    the inner messages as they are created, and the :class:`~agentdhal_agentchat.base.TaskResult`
    object as the last item before closing the generator.
    ...
    **Structured output:**
    ...
    If the `output_content_type` is set, the agent will respond with a :class:`~agentdhal_agentchat.messages.StructuredMessage`
    instead of a :class:`~agentdhal_agentchat.messages.TextMessage` in the final response by default.
    ...
    * If a handoff is triggered, a :class:`~agentdhal_agentchat.messages.HandoffMessage` will be returned in :attr:`~agentdhal_agentchat.base.Response.chat_message`.
    * If there are tool calls, they will also be executed right away before returning the handoff.
    * The tool calls and results are passed to the target agent through :attr:`~agentdhal_agentchat.messages.HandoffMessage.context`.
    ...
    **Limit context size sent to the model:**
    ...
    Another option is to use a :class:`~agentdhal_core.model_context.TokenLimitedChatCompletionContext`
    ...
    **Streaming mode:**
    ...
    :class:`~agentdhal_agentchat.messages.ModelClientStreamingChunkEvent`
    ...
    Args:
        ...
        handoffs (List[HandoffBase | str] | None, optional): The handoff configurations for the agent,
            ...
            The transfer is only executed when the team is in :class:`~agentdhal_agentchat.teams.Swarm`.
        ...
        model_client_stream (bool, optional): If `True`, the model client will be used in streaming mode.
            :meth:`on_messages_stream` and :meth:`BaseChatAgent.run_stream` methods will also yield :class:`~agentdhal_agentchat.messages.ModelClientStreamingChunkEvent`
        ...
        output_content_type (type[BaseModel] | None, optional): The output content type for :class:`~agentdhal_agentchat.messages.StructuredMessage` response as a Pydantic model.
            This will be used with the model client to generate structured output.
            If this is set, the agent will respond with a :class:`~agentdhal_agentchat.messages.StructuredMessage` instead of a :class:`~agentdhal_agentchat.messages.TextMessage`
            in the final response, unless `reflect_on_tool_use` is `False` and a tool call is made.
        output_content_type_format (str | None, optional): (Experimental) The format string used for the content of a :class:`~agentdhal_agentchat.messages.StructuredMessage` response.
        max_tool_iterations (int, optional): The maximum number of tool iterations to perform until the model stops making tool calls. Defaults to `1`, which means the agent will
            only execute the tool calls made by the model once, and return the result as a :class:`~agentdhal_agentchat.messages.ToolCallSummaryMessage`,
            or a :class:`~agentdhal_agentchat.messages.TextMessage` or a :class:`~agentdhal_agentchat.messages.StructuredMessage` (when using structured output)
            in :attr:`~agentdhal_agentchat.base.Response.chat_message` as the final response.
        ...
        tool_call_summary_format (str, optional): Static format string applied to each tool call result when composing the :class:`~agentdhal_agentchat.messages.ToolCallSummaryMessage`.
    ...
    Examples:
        ...
        **Example 3: agent with tools**
        ...
        print the messages to the console using :class:`~agentdhal_agentchat.ui.Console`.

        The tool is a simple function that returns the current time.
        Under the hood, the function is wrapped in a :class:`~agentdhal_core.tools.FunctionTool`
        ...
        **Example 5: agent with Model-Context Protocol (MCP) workbench**
        ...
        The following example demonstrates how to create an assistant agent with
        a model client and an :class:`~agentdhal_extensions.tools.mcp.McpWorkbench` for
        ...
        .. code-block:: python
            ...
            from agentdhal_agentchat.ui import Console
            from agentdhal_extensions.models.openai import OpenAIChatCompletionClient
            from agentdhal_extensions.tools.mcp import StdioServerParams, McpWorkbench
        ...
        **Example 6: agent with structured output and tool**
        ...
        Note that you need to use :class:`~agentdhal_core.tools.FunctionTool` to create the tool
        ...
        **Example 7: agent with bounded model context**
        ...
        :class:`~agentdhal_core.model_context.BufferedChatCompletionContext`
        ...
        **Example 8: agent with memory**
        ...
        before making an inference, using the :meth:`~agentdhal_core.memory.Memory.update_context` method.
        ...
        **Example 10: agent using reasoning model with custom model context.**
        ...
        .. code-block:: python
            ...
            from agentdhal_core.models import AssistantMessage, LLMMessage, ModelFamily
            from agentdhal_extensions.models.ollama import OllamaChatCompletionClient
    ...
    component_provider_override = "agentdhal_agentchat.agents.AssistantAgent"
